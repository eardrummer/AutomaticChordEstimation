...
HOP_LENGTH = 2048
OUTPUT_PATH = '../resources/realtime/'
...
class SWHear(object):
    ...
    def __init__(self,device=None,startStreaming=True):
        ...
        self.tapeLength=0.25 #Seconds
        #self.tape=np.empty(self.rate*self.tapeLength)*np.nan
        self.tape=np.empty(int(self.rate*self.tapeLength))*0
    ...
    # the newest data is always at the end. Don't modify data on the tape,
    ...
    def tape_forever(self,plotSec=.15):
        t1=0
        try:
            while True:
                self.tape_add()
                if (time.time()-t1)>plotSec:
                    t1=time.time()
                    self.tape_plot()
                    #self.tape_chromaVector_plot()
        except KeyboardInterrupt:
            print(" ~~ exception (keyboard?)")
            return

    def tape_plot(self,saveAs=os.path.join(OUTPUT_PATH,"waveform.png")):
        ...
        if saveAs:
            ...
            print("plotting waveform saving took %.02f ms"%((time.time()-t1)*1000))
        else:
            pylab.show()
            print() #good for IPython


        # Processing on every frame starts here

        #Check if tape is populated by proper values, catch initial few chunks when tape isnt fully populated
        if not np.isnan(self.tape).any():
            t1=time.time()

            CS.getChordsRealtime(audio=self.tape, 
            					sampleRate=self.rate,
            					hopLength=HOP_LENGTH,
            					imagePath=OUTPUT_PATH)

            print("Chroma calculation took %.02f ms"%((time.time()-t1)*1000))
